...
var AccountAPI = function () {
	...
	var urlListarAPIKeys = "/Account/ListarAPIKeys/";
	var urlTwoSteps = "/Account/TwoSteps/";
	var urlResendSecurityCode = "/Account/ResendSecurityCode/";

	var dataTableAPIKeys = null;
	var dataTableAcessos = null;
	...
	var _LogInForm = function () {
		...
		$('#kt_sign_in_submit').on('click', function (e) {
			var target = document.querySelector("#div_Login");
			...
			validation.validate().then(function (status) {
				if (status == 'Valid') {
					...
					var nome = $('#kt_sign_in_form input[name="nome"]').val();
	...
	var _ProfileForm = function (e) {
		...
		if ($('#APIKeys')) {
			...
			var options = _DefaultDataTablesOptions(urlListarAPIKeys);
			...
			options.columns = [
				...
				},	
				{
					...
					render: function (data, type, row) {
						...
						return `<!--begin::Input wrapper-->
								<div class="w-100">									
									<!--begin::Title-->
									<div class="d-flex justify-content-center">										
										<button class="btn btn-light btn-sm fw-bold flex-shrink-0 copy-button" data-clipboard-text="${row.Key}">Copiar</button>
									</div>
									<!--end::Title-->
								</div>
								<!--end::Input wrapper-->`;
			...
			dataTableAPIKeys = $('#kt_datatable_api_keys').DataTable(options);

			dataTableAPIKeys.on('draw', function () {
			...
			dataTableAPIKeys.on('error.dt', function () {
			...
			$('#kt_datatable_search_api_keys').on('keyup', function (e) {

				dataTableAPIKeys.search(e.target.value).draw();
		...
		if ($('#Acessos')) {
			...
			dataTableAcessos = $('#kt_datatable_acessos').DataTable(options);

			dataTableAcessos.on('draw', function () {

				KTMenu.createInstances();

			});

			dataTableAcessos.on('error.dt', function () {
			...
			$('#kt_datatable_search_acessos').on('keyup', function (e) {

				dataTableAcessos.search(e.target.value).draw();
	...
	var onClickBtnCopy = function () {

		$('.copy-button').each(function (index) {
