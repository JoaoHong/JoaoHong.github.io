...
namespace ConyGreen.DAO.Repository
{
    ...
    where TEntity : class
    where TContext : IdentityDbContext<AspNetUsers>, new()
    {
        ...
        public virtual bool BulkInsert(IEnumerable<TEntity> entities, int batchSize)
        {
            var success = true;
            ...
            executionStrategy.Execute(
                () =>
                {
                    ...
                    try
                    {
                        context.BulkInsert(entities.ToList(), new BulkConfig { SetOutputIdentity = true, BatchSize = batchSize, IncludeGraph = true });
                        transaction.Commit();
                    }
                    catch (Exception e)
                    {
        ...
        public virtual bool BulkUpdate(IEnumerable<TEntity> entities, int batchSize)
        {
            var success = true;
            ...
            executionStrategy.Execute(
                () =>
                {
                    ...
                    try
                    {
                        context.BulkUpdate(entities.ToList(), new BulkConfig { SetOutputIdentity = true, BatchSize = batchSize, IncludeGraph = true });
                        transaction.Commit();                        
                    }
                    catch (Exception e)
                    {
